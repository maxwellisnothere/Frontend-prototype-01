...
/** Lightweight glow — two LinearGradients scoped inside imageBox, no extra libs */
const GlowBackground = ({ color }) => (
  <View style={StyleSheet.absoluteFill} pointerEvents="none">
    ...
    <LinearGradient
      ...
      style={[StyleSheet.absoluteFill, { margin: 20, borderRadius: 40 }]}
    />
  </View>
);
  ...
  return (
    <TouchableOpacity style={cs.card} onPress={onPress} activeOpacity={0.88}>
      ...
      <View style={cs.imageBox}>
        {/* lightweight SVG glow — replaces BlurView stack */}
      ...
      <View style={cs.info}>
        ...
        <Text style={cs.price}>
          ฿{(item.listingPrice || 0).toLocaleString("th-TH", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}
        </Text>
        {item.sellerName && (
          <Text style={cs.seller}>Listed by {item.sellerName}</Text>
        )}
      </View>
...
export default function StoreScreen({ navigation }) {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (selectedCategory)
      result = result.filter((i) =>
        (i.category || i.type || "").toLowerCase().includes(selectedCategory.toLowerCase())
      );
    ...
    if (filters.sort === "price_asc")
      result.sort((a, b) => (a.listingPrice || 0) - (b.listingPrice || 0));
    else if (filters.sort === "price_desc")
      result.sort((a, b) => (b.listingPrice || 0) - (a.listingPrice || 0));
  ...
  // ── Stable data loader — prevents jank from duplicate calls ──
  const loadData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      if (data.success) {
        // Batch-enrich off the main thread tick so FlatList doesn't stutter
  ...
  const keyExtractor = useCallback(
    (item, idx) => item.listingId || item.id || String(idx),
    []
  );
  ...
  return (
    <View style={s.container}>
      ...
      {/* ── Layer 1: Subtle top-left accent (primary color bleed) ── */}
      <LinearGradient
        colors={["rgba(236,100,108,0.13)", "transparent"]}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
            ...
            {CATEGORIES.map((cat) => (
              ...
              >
                {selectedCategory === cat.id && (
                  <LinearGradient
                    colors={["rgba(236,100,108,0.22)", "transparent"]}
                    style={StyleSheet.absoluteFill}
                  />
                )}
                <Feather
        ...
        <View style={s.resultRow}>
          <Text style={s.resultText}>
            {activeFilterCount > 0 || search || selectedCategory
              ...
              : `TOTAL ${items.length} ITEMS FOR SALE`}
          </Text>
          {activeFilterCount > 0 && (
...
const s = StyleSheet.create({
  ...
  headerTitle: {
    color: colors.textPrimary, fontFamily: "Rajdhani_700Bold",
    fontSize: 20, fontStyle: "italic", letterSpacing: 1,
  },
  ...
  searchBox: {
    ...
    backgroundColor: "rgba(255,255,255,0.05)", borderRadius: 12, height: 44,
    borderWidth: 0.5, borderColor: "rgba(255,255,255,0.12)", overflow: "hidden",
  },
  searchIcon: { paddingLeft: 14 },
  searchInput: {
    flex: 1, color: colors.textPrimary, fontFamily: "Rajdhani_600SemiBold",
  ...
  catBtn: {
    ...
    borderRadius: 20, backgroundColor: "rgba(7,11,20,0.5)", borderWidth: 0.5,
    borderColor: "rgba(255,255,255,0.1)", overflow: "hidden",
  },
  catBtnActive: { borderColor: colors.primary },
  catText: {
    color: colors.textSecondary, fontFamily: "Rajdhani_600SemiBold",
    fontSize: 11, letterSpacing: 0.5, marginLeft: 6, zIndex: 10,
  },
...
const cs = StyleSheet.create({
  card: {
    ...
    backgroundColor: "rgba(76,44,52,0.2)", borderWidth: 0.5,
    borderColor: "rgba(255,255,255,0.1)", position: "relative",
  },
