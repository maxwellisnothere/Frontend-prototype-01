...
import {
  ...
  FlatList, Alert, ActivityIndicator, RefreshControl, Image, ScrollView,
} from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { colors } from "../theme/colors";
import FilterModal, { DEFAULT_FILTERS } from "../components/FilterModal";
import { fetchListings } from "../data/api"; // ✅ เอา fetchItems ออก เหลือแค่ดึงของที่วางขาย
    ...
    <View style={cs.info}>
      ...
      {item.wear && (
        <View style={cs.wearRow}>
          ...
          <Text style={[cs.wearText, { color: item.wearColor || "#888" }]}>{item.wear}</Text>
        </View>
      )}
      {item.float !== undefined && (
        <Text style={cs.floatText}>Float: {Number(item.float).toFixed(4)}</Text>
      )}
      {/* ✅ แสดงราคาวางขาย พร้อมทศนิยม */}
      <Text style={cs.price}>
...
export default function StoreScreen({ navigation }) {
  ...
  const filteredItems = useMemo(() => {
    ...
    // กรองหมวดหมู่
    if (selectedCategory) {
      result = result.filter(i => (i.category || i.type || "").toLowerCase().includes(selectedCategory.toLowerCase()));
    }
  ...
  const loadData = async (isRefresh = false) => {
    ...
    try {
      // ✅ ยิง API ดึงเฉพาะของที่ตั้งขายเท่านั้น
      const data = await fetchListings();
      if (data.success) {
        setItems(
          data.listings.map((l) => enrichItem({
            ...l.item,
            listingId: l.listingId || l._id,
    ...
    <SafeAreaView style={s.safe} edges={["top"]}>
      ...
      <View style={s.resultRow}>
        <Text style={s.resultText}>
          {activeFilterCount > 0 || search || selectedCategory
            ? `พบ ${filteredItems.length} ชิ้น (จาก ${items.length} รายการที่วางขาย)`
            : `วางขายทั้งหมด ${items.length} รายการ`}
        </Text>
        {activeFilterCount > 0 && (
      ...
      {loading ? (
        <View style={s.loadingBox}>
          ...
          <Text style={s.loadingText}>กำลังโหลดตลาด...</Text>
        </View>
      ) : (
        <FlatList
          data={filteredItems}
            ...
            <ItemCard
              ...
              onPress={() =>
                navigation.navigate("ItemDetail", {
                  // ✅ ส่ง price ไปทับให้ ItemDetail แสดงราคาวางขายของจริง
                  item: { ...item, price: item.listingPrice }, 
                })
              }
            />
          )}
          ListEmptyComponent={
            <View style={s.empty}>
              ...
              <Text style={s.emptyText}>ไม่มีคนวางขายไอเทมนี้เลย</Text>
            </View>
          }
        />
      )}
...
const s = StyleSheet.create({
  ...
  filterBtn: { padding: 6, borderRadius: 8, borderWidth: 1, borderColor: "#333", position: "relative" },
  filterBtnActive: { borderColor: colors.primary, backgroundColor: colors.primary + "22" },
  filterIcon: { fontSize: 16 },
  filterBadge: { position: "absolute", top: -4, right: -4, backgroundColor: colors.primary, borderRadius: 8, minWidth: 16, height: 16, justifyContent: "center", alignItems: "center", paddingHorizontal: 3 },
  filterBadgeText: { color: "#000", fontSize: 9, fontWeight: "900" },
  clearFilterText: { color: colors.primary, fontSize: 11, fontWeight: "700" },
  balanceBadge: { backgroundColor: colors.primary + "22", borderRadius: 20, paddingHorizontal: 12, paddingVertical: 4, borderWidth: 1, borderColor: colors.primary + "44" },
  balanceText: { color: colors.primary, fontSize: 13, fontWeight: "800" },
  searchRow: { flexDirection: "row", paddingHorizontal: 12, paddingVertical: 8, gap: 8, backgroundColor: colors.surface, borderBottomWidth: 1, borderBottomColor: colors.border },
  searchBox: { flex: 1, flexDirection: "row", alignItems: "center", backgroundColor: colors.surfaceElevated, borderRadius: 10, paddingHorizontal: 10, borderWidth: 1, borderColor: colors.border },
  ...
  searchBtn: { backgroundColor: colors.primary, borderRadius: 10, paddingHorizontal: 14, justifyContent: "center" },
  searchBtnText: { color: "#000", fontSize: 13, fontWeight: "800" },
  catScroll: { backgroundColor: colors.surface, borderBottomWidth: 1, borderBottomColor: colors.border, flexShrink: 0, flexGrow: 0 },
  catContent: { paddingHorizontal: 12, paddingVertical: 8, flexDirection: "row", alignItems: "center" },
  catBtn: { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20, backgroundColor: colors.surfaceElevated, borderWidth: 1, borderColor: colors.border, marginRight: 8, flexShrink: 0 },
  ...
  resultRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 16, paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: colors.border },
...
const cs = StyleSheet.create({
  card: { flex: 1, margin: 4, backgroundColor: colors.cardBg, borderRadius: 10, overflow: "hidden", borderWidth: 1, borderColor: colors.border },
  rarityBar: { height: 3 },
  imageBox: { height: 110, backgroundColor: colors.surfaceElevated, alignItems: "center", justifyContent: "center" },
  ...
  rarityBadge: { borderWidth: 1, borderRadius: 4, paddingHorizontal: 5, paddingVertical: 1, alignSelf: "flex-start", marginBottom: 4 },
  ...
  price: { color: colors.primary, fontSize: 13, fontWeight: "900", marginTop: 4 },
  seller: { color: colors.textMuted, fontSize: 9, marginTop: 4 },
});